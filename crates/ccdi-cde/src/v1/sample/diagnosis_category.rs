...
pub enum DiagnosisCategory {
    ...
    AtypicalTeratoidOrRhabdoidTumor,
    ...
    /// * **Begin Date**:   02/05/2024
    ///
    /// A group of central nervous system neoplasms with a variable amount of neuronal and, less consistently, glial differentiation. They occur at a low frequency and usually carry a favorable prognosis.  Representative examples include dysplastic cerebellar gangliocytoma, desmoplastic infantile ganglioglioma, desmoplastic infantile astrocytoma, and dysembryoplastic neuroepithelial tumor. (Adapted from WHO)
    ...
    ///
    /// A grade 3 or grade 4 glioma arising from the central nervous system. This category includes glioblastoma, anaplastic astrocytoma, anaplastic ependymoma, anaplastic oligodendroglioma, and anaplastic oligoastrocytoma.
    ...
    ///
    /// A term that refers to central nervous system embryonal tumors which are not fully characterized.
    ...
    /// * **Begin Date**:   10/20/2025
    ///
    /// A benign, premalignant, or malignant neoplasm that affects the liver parenchyma or intrahepatic bile ducts.
...
impl std::fmt::Display for DiagnosisCategory {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            DiagnosisCategory::AtypicalTeratoidOrRhabdoidTumor => {
...
impl Distribution<DiagnosisCategory> for Standard {
    fn sample<R: rand::Rng + ?Sized>(&self, rng: &mut R) -> DiagnosisCategory {
        match rng.gen_range(0..=23) {
            0 => DiagnosisCategory::AtypicalTeratoidOrRhabdoidTumor,
...
mod tests {
    ...
    fn it_converts_to_string_correctly() {
        assert_eq!(
            DiagnosisCategory::AtypicalTeratoidOrRhabdoidTumor.to_string(),
            "Atypical Teratoid/Rhabdoid Tumor"
        );
        assert_eq!(
    ...
    fn it_serializes_to_json_correctly() {
        assert_eq!(
            serde_json::to_string(&DiagnosisCategory::AtypicalTeratoidOrRhabdoidTumor).unwrap(),
            "\"Atypical Teratoid/Rhabdoid Tumor\""
        );
        assert_eq!(
