...
impl Sample {
    ...
    pub fn new(
        id: Identifier,
        ancestor: cde::v1::subject::Identifier,
        metadata: Option<Metadata>,
    ) -> Self {
        Self {
            id,
    ...
    /// );
    ///
    /// assert_eq!(sample.ancestor().namespace(), &String::from("organization"));
    /// assert_eq!(sample.ancestor().name(), &String::from("SubjectName001"));
    ///
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn ancestor(&self) -> &cde::v1::subject::Identifier {
        &self.subject
    }
    ...
    /// );
    /// ```
    pub fn random(identifier: Identifier, ancestor: cde::v1::subject::Identifier) -> Self {
